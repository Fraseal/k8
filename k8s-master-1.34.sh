...
CALICO_VERSION=v3.29.0   # <-- Official Calico with full NetworkPolicy support
...
### install containerd 2.2 over apt-installed version
...
### CNI - Calico with full NetworkPolicy support (Official source)
...
# Step 3: Apply Calico custom resources (defines pod CIDR + enables NetworkPolicy)
...
spec:
  calicoNetwork:
    ipPools:
    - blockSize: 26
      cidr: 192.168.0.0/16        # Must match --pod-network-cidr above
...
# Step 4: Wait for Calico pods to be Running
echo "Waiting for Calico pods to be ready (this may take 2-3 minutes)..."
kubectl wait --for=condition=ready pod -l k8s-app=calico-node -n calico-system --timeout=180s || true

echo ""
echo "### Verifying Calico NetworkPolicy is active ###"
