...
### NOTE: Worker node does NOT run kubeadm init or install CNI
### Calico CNI is automatically deployed by the master to all worker nodes
### once the worker joins the cluster via the join command below.

echo
echo "### READY TO JOIN CLUSTER ###"
echo "Run the kubeadm join command from your master node output."
echo "Example (replace with your actual token and cert hash from master):"
echo ""
echo "  kubeadm join <MASTER_IP>:6443 \\"
echo "    --token <TOKEN> \\"
echo "    --discovery-token-ca-cert-hash sha256:<HASH>"
echo ""
echo "To get the join command from master, run on master node:"
